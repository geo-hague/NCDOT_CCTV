// combine-tokens.js — unions the per-shard token files (downloaded into ./parts)
// into a single tokens.json. Run before merge-tokens.js in the merge job.
const fs = require('fs');
  ...
  for (const e of fs.readdirSync(d, { withFileTypes: true })) {
    ...
    if (e.isDirectory()) walk(p);
      ...
      try {
        ...
        for (const en of (j.entries || [])) if (en.chan) all[en.chan] = en;
        files++;
      } catch (e) {}
    }
  }
